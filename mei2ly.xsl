...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:m="http://www.bach-digital.de/m" xmlns:saxon="http://saxon.sf.net/" exclude-result-prefixes="saxon">
  ...
  <xsl:template match="mei:beam">
    <xsl:if test="@color">
      <xsl:value-of select="concat('\once \override Beam.color = #(x11-color &quot;',@color,'&quot;) ')"/>
    </xsl:if>
    <xsl:apply-templates/>
  </xsl:template>
  <!-- MEI tuplet elements -->
  ...
  <xsl:template match="mei:slur" mode="pre">
    <xsl:if test="@color">
      <xsl:value-of select="concat('\once \override Slur.color = #(x11-color &quot;',@color,'&quot;) ')"/>
    </xsl:if>
    <xsl:if test="@lform">
  ...
  <xsl:template match="mei:tempo" mode="pre">
    <xsl:if test="@color">
      <xsl:value-of select="concat('\once \override Score.MetronomeMark.color = #(x11-color &quot;',@color,'&quot;) ')"/>
    </xsl:if>
